    ...
    namespace mapper
    {
        ...
        Reward Env::calculate_reward()
        {
            ...
            else
            {
                ...
                analyzer_.analyze();
                if (obj_ == Objective::CYCLE)
                    reward.value = (double)(analyzer_.get_cycle());
                else if (obj_ == Objective::ENERGY)
                    reward.value = (double)(analyzer_.get_energy());
        ...
        void Env::insert(const SymbolTable *table, double value)
        {
            ...
            while (iter != topKList_.end() && value > iter->second)
                iter++;
            topKList_.insert(iter, {table, value});
            if (topKList_.size() > topk_)
                topKList_.pop_back();
        }
        ...
        std::ostream &operator<<(std::ostream &o, const State &s)
        {
            ...
            o << "\tCandidates: ";
            for (auto factor : s.candidate_factors_)
                o << factor << ",";
            o << std::endl;
